...
- Installed Ollama models included the documented model tags plus additional models (`ollama list`): `llama3:latest`, `mistral:latest`, `deepseek-r1:latest`, `phi3:latest`, `qwen3:latest`, plus `llama3.2-vision`, `codegemma`, `deepseek-coder`, `qwen2.5-coder`, `granite4`, `gemma3`, and `phi4`.
...
| `deepseek-r1` | `deepseek-r1:latest` | 8.2B | Q4_K_M | 131072 | `temperature=0.6`, `top_p=0.95`; stop tokens `<｜begin▁of▁sentence｜>`, `<｜end▁of▁sentence｜>`, `<｜User｜>`, `<｜Assistant｜>` (`ollama show deepseek-r1`; `ollama show --parameters deepseek-r1`) | DeepSeek chat template with user/assistant role tokens, tool support, and thinking-output handling (`ollama show --template deepseek-r1`) |
...
- Model tags are recorded for documentation as `llama -> llama3`, `mistral -> mistral`, `deepseek-r1 -> deepseek-r1`, `phi3 -> phi3`, `qwen3 -> qwen3`; model-name resolution is implemented by `load_model_config` and `get_model_config` (`src/utils.py:225-291`).
...
- The documented DeepSeek tag should be kept synchronized across `models/models.json`, README examples, and seed-analysis model subsets before rerunning all configured models (`models/models.json:2-6`; `README.md:77-87`; `seed_analysis.py:14`; `src/utils.py:271-291`).
- The code passes only `temperature`, `num_predict`, and `seed` to Ollama (`src/llm_detector.py:89-93`). Model-level defaults therefore differ across models for unpassed parameters: for example, `qwen3` has `top_k=20`, `top_p=0.95`, `repeat_penalty=1`, and `deepseek-r1` has `temperature=0.6`, `top_p=0.95`, and DeepSeek stop tokens (`ollama show --parameters qwen3`; `ollama show --parameters deepseek-r1`), while Llama/Mistral/Phi show no comparable sampling parameters.
